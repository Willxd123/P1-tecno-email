...
public class PPedidos {
    ...
    public static String generarHtml(String comando, String resultado) {
        String tituloModulo = "Gestión de Pedidos y Pagos - “CHIFONES PERUANOS ZUZÚ”";
    ...
    private static String parsearTablaAscii(String asciiText) {
        ...
        if (!lineas[0].contains("|") && lineas[0].length() > 0) {
            htmlTable.append("<h3 style=\"color: #2b2b2b; margin-bottom: 10px;\">").append(lineas[0]).append("</h3>");
            startIdx = 1;
        ...
        for (int i = startIdx; i < lineas.length; i++) {
            ...
            if (!linea.contains("|")) {
                // Si es la línea de resumen de totales del detalle del pedido
                htmlTable.append("</table>");
            ...
            for (int col = 0; col < celdas.length; col++) {
                ...
                if (esCabecera) {
                    ...
                    htmlTable.append("<td>");
                    // Aplicar badges a estados de pedido y pagos
                    if (celdaText.equalsIgnoreCase("PAGADO") || celdaText.equalsIgnoreCase("ENTREGADO") || celdaText.equalsIgnoreCase("SÍ")) {
                        htmlTable.append("<span class=\"badge badge-si\">").append(celdaText.toUpperCase()).append("</span>");
                    } else if (celdaText.equalsIgnoreCase("PENDIENTE") || celdaText.equalsIgnoreCase("NO")) {
                        htmlTable.append("<span class=\"badge badge-pend\">").append(celdaText.toUpperCase()).append("</span>");
                    } else if (celdaText.equalsIgnoreCase("CANCELADO")) {
                        htmlTable.append("<span class=\"badge badge-no\">CANCELADO</span>");
                    } else {
